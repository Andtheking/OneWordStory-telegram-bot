...
class Partita:
    ...
    def getAllPartecipants(self, stringa: bool) -> list[str] | str:
        lista = []
...
def join_ows_game(update: Update, context: CallbackContext):
    ...
    for partecipante in partite[f'{chat_id}'].partecipanti:
        logger.info(
            f"{partecipante} ({partite[f'{chat_id}'].partecipanti[f'{partecipante}'].nomeUtente})")
...
def avvia_partita(update: Update, context: CallbackContext):
    ...
    if not idUtente == partite[f'{chat_id}'].leader:
...
def salvaVariabile(variabile, nomeFile):
    with open(nomeFile,'wb') as File:
        pickle.dump(variabile,File)

def caricaVariabile(nomeFile):
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.chat_type.groups & ~
                   Filters.command, onMessageInGroup, run_async=True))
